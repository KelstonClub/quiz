...
def parse(db, csv_file):
    ...
    for line in csv_file:
        ...
        print(token, args)
        if token == "quizzes":
            rval = db.conn.execute("INSERT INTO quizzes(name) VALUES (?)", args).lastrowid
        
        elif token == "questions":
            qid = ids["quizzes"][int(args[0]) - 1]
            rval = db.conn.execute("insert into questions(quiz_id, text, type) VALUES (?, ?, ?)", (qid, args[1], args[2])).lastrowid

        elif token == "answers":
            qid = ids["questions"][int(args[0]) - 1]
            rval = db.conn.execute("insert into answers(question_id, text, is_right) VALUES (?, ?, ?)", (qid, args[1], args[2]))
